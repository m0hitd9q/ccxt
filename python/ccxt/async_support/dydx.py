...
class dydx(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(dydx, self).describe(), {
            ...
            'api': {
                ...
                'private': {
                    'get': {
                        'rewards/public-retroactive-mining': 60,  # TODO: write to exchange API team, that self endpoint is listed under 'public' endpoints, but needs apikey
            ...
            # default credential
            'starkPrivateKey': None,
            'options': {
                'mainCurrency': 'USDC',
                'limitFee': '0.01',  # 1%  # TODO: self needs to be defined automatically, from either /users or /config endpoints, however we don't implement them
    ...
    async def fetch_markets(self, params={}):
        ...
        settleId = self.options['mainCurrency']  # https://docs.dydx.exchange/#margin
        for i in range(0, len(keys)):
            marketKey = keys[i]
            market = markets[marketKey]
            id = self.safe_string(market, 'market', marketKey)
            ...
            stepSize = self.safe_number(market, 'stepSize')
            tickSize = self.safe_number(market, 'tickSize')
            minOrderSize = self.safe_number(market, 'minOrderSize')
    ...
    async def fetch_trading_fees(self, params={}):
        ...
        response = await self.publicGetConfig(params)
        # {
        ...
        result = {}
        maker = self.safe_number(response, 'defaultMakerFee')
        taker = self.safe_number(response, 'defaultTakerFee')
    ...
    async def fetch_order_book(self, symbol, limit=None, params={}):
        ...
        # }
        #
        return self.parse_order_book(response, symbol, None, 'bids', 'asks', 'price', 'size')
    ...
    def parse_trade(self, trade, market):
        ...
        type = self.parse_order_type(self.safe_string(trade, 'type'))
        timestamp = self.parse_date(self.safe_string(trade, 'createdAt'))
        side = self.parse_order_side(self.safe_string(trade, 'side'))
        takerOrMaker = self.parse_taker_maker(self.safe_string(trade, 'liquidity'))
        fee = None
        cost = self.safe_number(trade, 'fee')
        if cost is not None:
            fee = {
                'cost': cost,
                'currency': self.options['mainCurrency'],
            }
    ...
    async def fetch_ohlcv(self, symbol, timeframe='1m', since=None, limit=None, params={}):
        ...
        response = await self.publicGetCandlesMarket(self.extend(request, params))
        #
        # {
        #   "candles": [
        #     {
        ...
        # }
        #
        data = self.safe_value(response, 'candles', [])
        return self.parse_ohlcvs(data, market, timeframe, since, limit)

    def parse_ohlcv(self, ohlcv, market=None):
        startedAt = self.safe_string(ohlcv, 'startedAt')
        timestamp = self.parse_date(startedAt)
        ...
        # volume = self.safe_number(ohlcv, 'usdVolume')
        baseTokenVolume = self.safe_number(ohlcv, 'baseTokenVolume')
    ...
    async def fetch_tickers(self, symbols=None, params={}):
        ...
        response = await self.publicGetStatsMarket(self.extend(request, params))
        # {
        #     markets: {
        #       'ETH-USD': {
        #         market: 'ETH-USD',
        ...
        # }
        #
        data = self.safe_value(response, 'markets', [])
    ...
    async def fetch_ticker(self, symbol, params={}):
        ...
        ticker = self.safe_value(markets, market['id'], {})
    ...
    async def fetch_balance(self, params={}):
        ...
        code = self.options['mainCurrency']
        allAccounts = self.account()
        allAccounts['free'] = '0'
        allAccounts['total'] = '0'
        for i in range(0, len(accounts)):
            currentAcc = accounts[i]
            account = self.account()
            account['free'] = self.safe_string(currentAcc, 'freeCollateral')
            account['total'] = self.safe_string(currentAcc, 'equity')
            if account['free']:
    ...
    async def fetch_position(self, symbol, params={}):
        ...
        request['status'] = self.parse_position_status('open', True)
    ...
    async def fetch_positions(self, symbols=None, params={}):
        ...
        return self.parse_positions(data, symbols, params)

    def parse_position_status(self, status, reversed=False):
        statuses = {
            'OPEN': 'open',
            'CLOSED': 'closed',
        }
        if reversed:
            statuses = self.parse_array_transpose(statuses)
        return self.safe_string(statuses, status, status)

    def parse_position_side(self, status, reversed=False):
        statuses = {
            'LONG': 'long',
            'SHORT': 'short',
        }
    ...
    def parse_position(self, position, market=None):
        ...
        market = self.safe_market(marketId, market)
        status = self.safe_string(position, 'status') == 'open' if 'OPEN' else 'closed'
        ...
        return {
            ...
            'exitPrice': self.safe_number(position, 'exitPrice'),
            'status': status,
            'notional': None,
            'leverage': None,
            ...
            'contracts': size,
            'contractSize': None,
            'realisedPnl': self.safe_number(position, 'realizedPnl'),
            'marginRatio': None,
            'liquidationPrice': None,
            'markPrice': None,
            'collateral': self.options['mainCurrency'],
            'marginType': None,
            'side': side,
            'percentage': None,
            'info': position,
    ...
    async def fetch_transactions(self, code=None, since=None, limit=None, params={}):
        ...
        #   ]
        # }
        data = self.safe_value(response, 'transfers')
        return self.parse_transactions(data, currency, since, limit)

    def parse_transaction_status(self, status, reversed=False):
        statuses = {
            'PENDING': 'pending',
            'CONFIRMED': 'ok',
        }
    ...
    def parse_transaction_type(self, status, reversed=False):
        statuses = {
            'DEPOSIT': 'deposit',
            'WITHDRAWAL': 'withdraw',
    ...
    def parse_transaction(self, transaction, currency=None):
        ...
        return {
            ...
            'amount': self.safe_number(transaction, 'creditAmount'),
            'type': self.parse_transaction_type(self.safe_string(transaction, 'type')),
            'currency': self.safe_currency_code(self.safe_string(transaction, 'creditAsset')),
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        passedPrice = price if price else self.safe_number(params, 'price')
        if not passedPrice:
            raise ArgumentsRequired(self.id + ' createOrder() always requires a set price, i.e. the worst accepted price of the base asset in USD, even for a market order. If you are using a derived order function such as createMarketBuyOrder() or createMarketSellOrder(), that do not take a price argument, you may pass the price in the params argument.')
        self.check_stark_key_credentials()
        await self.load_markets()
        market = self.market(symbol)
        request = {
            'market': market['id'],
            'side': self.parse_order_side(side, True),
            'type': self.parse_order_type(type, True),
        }
        request['size'] = self.amount_to_precision(market['symbol'], amount)
        request['price'] = self.price_to_precision(market['symbol'], passedPrice)
        # [Required] limitFee
        request['limitFee'] = self.safe_string(params, 'limitFee', self.options['limitFee'])
        # timeInForce for market orders
        if type == 'market':
            request['timeInForce'] = 'FOK'
        request['timeInForce'] = self.safe_string(params, 'timeInForce', request['timeInForce'])
        # [Required] expiration
        request['expiration'] = self.safe_string(params, 'expiration', self.options['gtcDate'])
        # [Required] postOnly
        request['postOnly'] = self.safe_value(params, 'postOnly', False)
        # [Required] clientId
        clientOrderId = self.safe_string_2(params, 'clientId', 'clientOrderId')
        request['clientId'] = clientOrderId if (clientOrderId is not None) else str(self.milliseconds())
        orderToSign = {
        ...
        hashHex = await self.stark_hash_hex(orderToSign, 'order')
        signature = await self.stark_signature(hashHex)
        request['signature'] = signature
        params = self.omit(params, ['clientOrderId', 'price'])
    ...
    async def cancel_order(self, id, symbol=None, params={}):
        ...
        request = None
        order = None
        # if symbol is inputed, then use self endpoint because of higher Ratelimits(yeah, bad api design, but as is..): https://docs.dydx.exchange/#cancel-active-orders
        if symbol is not None:
            side = self.safe_string(params, 'side')
            if side is None:
                raise ArgumentsRequired(self.id + ' cancelOrder() does not require `symbol` argument, however, if you set that argument, then you also need to set exchange-specific parameter `side` to either: `BUY` or `SELL`.')
            market = self.market(symbol)
            request = {
                'id': id,
                'side': side,
            ...
            response = await self.privateDeleteActiveOrders(self.extend(request, params))
            # {
            #     "cancelOrders": [
            #         {
            ...
            #     ]
            # }
            orders = self.safe_value(response, 'cancelOrders')
            order = self.safe_value(orders, 0)
        else:
            request = {
                'orderId': id,
                'id': id,  # self is needed for path
            }
            ...
            #     },
            # }
            order = self.safe_value(response, 'cancelOrder')
        market = None
        if symbol is not None:
            market = self.market(symbol)
        else:
    ...
    async def cancel_all_orders(self, symbol=None, params={}):
        ...
        ordersArray = self.safe_value(response, 'cancelOrders', [])
        return self.parse_orders(ordersArray, market, None, None, params)

    def parse_order(self, order, market=None):
        marketId = self.safe_string(order, 'market')
        market = self.safe_market(marketId, market)
        timestamp = self.parse_date(self.safe_string(order, 'createdAt'))
        expiresAt = self.safe_string(order, 'expiresAt')
        timeInForce = self.safe_string(order, 'timeInForce')
        # simulate GTC like we did in order
        if expiresAt == self.options['gtcDate'] and timeInForce == 'GTT':
            timeInForce = 'GTC'
    ...
    def parse_order_status(self, status, reversed=False):
        statuses = {
            'PENDING': 'open',
            'OPEN': 'open',
            'UNTRIGGERED': 'open',
    ...
    def parse_order_type(self, status, reversed=False):
        statuses = {
            'MARKET': 'market',
            'LIMIT': 'limit',
    ...
    def parse_time_in_force(self, status, reversed=False):
        statuses = {
            'GTT': 'gtt',
            'FOK': 'fok',
            'IOC': 'ioc',
        ...
        if reversed:
            statuses = self.parse_array_transpose(status)
        return self.safe_string(statuses, status, status)

    def parse_order_side(self, status, reversed=False):
        statuses = {
            'BUY': 'buy',
            'SELL': 'sell',
        }
    ...
    def parse_taker_maker(self, status, reversed=False):
        statuses = {
            'MAKER': 'maker',
            'TAKER': 'taker',
        }
    ...
    async def fetch_order(self, id, symbol=None, params={}):
        ...
        if clientOrderId is not None:
            ...
            response = await self.privateGetOrdersClientId(self.extend(request, params))
        else:
            request = {'id': id}
    ...
    async def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        # if symbol is inputed, then use self endpoint because of higher Ratelimits(yeah, bad api design, but as is..): https://docs.dydx.exchange/#get-active-orders
        method = self.safe_string(self.options, 'fetchOpenOrdersMethod')
        method = self.safe_string(params, 'method', method)
        if symbol is not None and method == 'privateGetActiveOrders':
            market = self.market(symbol)
            request = {
                'market': market['id'],
            }
            response = await self.privateGetActiveOrders(self.extend(request, params))
            ords = self.safe_value(response, 'orders')
            return self.parse_orders(ords, market, since, limit, params)
        else:
    ...
    async def fetch_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        arrayOrders = self.safe_value(response, 'orders', [])
        return self.parse_orders(arrayOrders, market, since, limit, params)
    ...
    async def fetch_my_trades(self, symbol=None, since=None, limit=None, params={}):
        ...
        #     ],
        # }
        arrayFills = self.safe_value(response, 'fills')
        return self.parse_trades(arrayFills, market, since, limit)

    async def register_api_key(self, params=None):
        response = await self.privatePostApiKeys(params)
        return response

    async def make_onboard(self, params=None):
    ...
    def sign(self, path, api='private', method='GET', params={}, headers=None, body=None):
        ...
        if method == 'POST' and path == 'onboarding':
            ...
            raise NotSupported(self.id + ' not yet implemented.')
        elif (method == 'POST' or method == 'DELETE') and path == 'api-keys':
            # can not be implemented at the moment due to EIP-712 resp. ABI encoding missing
            raise NotSupported(self.id + ' does not currently support POST or DELETE requests to the api-keys endpoint.')
        elif api == 'private':
            self.check_api_key_credentials()
            timestamp = self.iso8601(self.milliseconds())
            auth = timestamp + method + request
            if method != 'GET':
                body = self.json(params)
                auth += body
            secret = self.base64_to_binary(self.urldecode_base64(self.secret))
            signature = self.hmac(self.encode(auth), secret, hashlib.sha256, 'base64')
            headers = {
                'DYDX-TIMESTAMP': timestamp,
                'DYDX-API-KEY': self.apiKey,
                'DYDX-PASSPHRASE': self.password,
    ...
    def to_quantums_helper(self, humanAmount, quantumSize, roundMode, assertIntegerResult):
        amountPrecise = Precise(humanAmount)
        quantumSizePrecise = Precise(quantumSize)
        remainder = amountPrecise.mod(quantumSizePrecise)
        zeroPrecise = Precise('0')
        if assertIntegerResult and not remainder.equals(zeroPrecise):
            raise BadRequest(self.id + ' toQuantumsHelper: Amount ' + humanAmount + 'is not a multiple of the quantum size ' + quantumSize)
        quotient = amountPrecise.div(quantumSizePrecise)
        return str(quotient)  # self.decimal_to_precision(quotientStr, roundMode, 0)

    async def stark_signature(self, hashHex):
        request = self.fix_stark_hash_length(hashHex)
        secret = self.normalize_hex32(self.starkPrivateKey)
        signature = self.ecdsa(request, secret, 'stark')  # could extract curve specifics to self file, if pre defined curves in elliptic/lib shall not be extended
        return self.normalize_hex32(signature['r']) + self.normalize_hex32(signature['s'])

    def fix_stark_hash_length(self, messageHash):
        hashLength = len(messageHash)
        if hashLength <= 62:
            return messageHash
        if hashLength != 63:
            raise BadRequest(self.id + ' fixStarkHashLength: Invalid hash length: ' + str(hashLength) + ' != 63')
        return self.hex_shift_bits_n(messageHash, 4, True)

    def num_to_hex(self, dec, size):
        return self.binary_to_base16(self.number_to_be(dec, size))

    async def stark_hash_hex(self, signObject, type):
        assetIdCollateralHex = self.urls['api']['data']['assetIdCollateralHex']
        positionId = await self.get_position_id()
        positionIdHex = self.num_to_hex(positionId, 8)
        nonceHash = self.hash(self.encode(signObject['clientId']), 'SHA256')
        nonceHex = self.remove0x_prefix(self.hex_mod(self.remove0x_prefix(nonceHash), '100000000'))
        quantumSizeColl = '1e-6'  # for USDC collateral only, as all markets are
        if type == 'order':
            isBuyingSynthetic = signObject['side'] == 'BUY'
            assetIdSyntheticHex = self.remove0x_prefix(signObject['assetIdSynthetic'])
            quantumSizeSyn = Precise.string_div('1', signObject['assetResolution'])
            quantumsAmountSynthetic = self.to_quantums_helper(signObject['humanSize'], quantumSizeSyn, ROUND_DOWN, True)  # TODO check if roundings are even needed
            humanAmountCollateral = Precise.string_mul(signObject['humanSize'], signObject['humanPrice'])
            roundMode = ROUND_UP if isBuyingSynthetic else ROUND_DOWN
            quantumsAmountCollateral = self.to_quantums_helper(humanAmountCollateral, quantumSizeColl, roundMode, False)
            quantumsAmountFee = Precise.string_mul(signObject['limitFee'], quantumsAmountCollateral)
            expirationEpochHoursInt1 = self.parse8601(signObject['expirationIsoTimestamp']) / 3600000
            expirationEpochHoursInt = int(math.ceil(expirationEpochHoursInt1)) + 168
            assetIdFeeHex = assetIdCollateralHex
            quantumsAmountSyntheticHex = self.num_to_hex(quantumsAmountSynthetic, 8)
            quantumsAmountCollateralHex = self.num_to_hex(quantumsAmountCollateral, 8)
            quantumsAmountFeeHex = self.num_to_hex(quantumsAmountFee, 8)
            expirationEpochHoursHex = self.num_to_hex(expirationEpochHoursInt, 4)
            assetIdSellHex, assetIdBuyHex = [assetIdCollateralHex, assetIdSyntheticHex] if isBuyingSynthetic else [assetIdSyntheticHex, assetIdCollateralHex]
            quantumsAmountSellHex, quantumsAmountBuyHex = [quantumsAmountCollateralHex, quantumsAmountSyntheticHex] if isBuyingSynthetic else [quantumsAmountSyntheticHex, quantumsAmountCollateralHex]
            orderPart1Hex = quantumsAmountSellHex + quantumsAmountBuyHex + quantumsAmountFeeHex + nonceHex.rjust(8, '0')
            orderPart2HexUnshifted = '3' + positionIdHex + positionIdHex + positionIdHex + expirationEpochHoursHex
            orderPart2Hex = self.hex_shift_bits_n(orderPart2HexUnshifted, 17, True)
            assetsHex = self.get_cacheable_pedersen_hash(self.get_cacheable_pedersen_hash(assetIdSellHex, assetIdBuyHex), assetIdFeeHex)
            hashHex = self.pedersen_hash(self.pedersen_hash(assetsHex, orderPart1Hex), orderPart2Hex)
            return hashHex

    def get_cacheable_pedersen_hash(self, leftString, rightString):
        self.pedersenHashCache[leftString] = self.safe_value(self.pedersenHashCache, leftString, {})
        self.pedersenHashCache[leftString][rightString] = self.safe_value(
            self.pedersenHashCache[leftString],
            rightString,
            self.pedersen_hash(leftString, rightString)
        )
        return self.pedersenHashCache[leftString][rightString]

    def normalize_hex32(self, hex):
        return self.remove0x_prefix(hex).lower().rjust(64, '0')

    def derive_stark_private_key(self, privateKey):
        encodedMessageString = self.urls['api']['data']['encodedMessageStrings']['starkKey']
        signatureObject = self.sign_hash(encodedMessageString, privateKey)
        signature = self.remove0x_prefix(signatureObject['r']) + self.remove0x_prefix(signatureObject['s']) + self.num_to_hex(signatureObject['v'], 1) + '00'
        binary = self.base16_to_binary(signature)
        hashedData = self.hash(binary, 'keccak')
        starkPrivateKey = self.hex_shift_bits_n(self.remove0x_prefix(hashedData), 5, False)
        self.starkPrivateKey = self.normalize_hex32(starkPrivateKey)

    def uuid_format_key(self, key):
        return '-'.join([
            key[0:8],
            key[8:12],
            key[12:16],
    ...
    async def get_position_id(self):
        positionId = self.safe_string(self.options, 'positionId')
        if positionId:
            return positionId
        self.check_stark_key_credentials()
        ...
        for i in range(0, len(accounts)):
            currentAcc = accounts[i]
            currentAccountNumber = self.safe_string(currentAcc, 'accountNumber')
            accountNumber = self.safe_string(self.options, 'accountNumber')
            if accountNumber and (currentAccountNumber == accountNumber):
                positionId = self.safe_string(currentAcc, 'positionId')
                self.options['positionId'] = positionId
                return self.options['positionId']
        return None

    def recover_default_api_credentials(self, privateKey):
        encodedMessageString = self.urls['api']['data']['encodedMessageStrings']['onboarding']
        signatureObject = self.sign_hash(encodedMessageString, privateKey)
        signature = self.remove0x_prefix(signatureObject['r']) + self.remove0x_prefix(signatureObject['s']) + self.num_to_hex(signatureObject['v'], 1) + '00'
        # secret
        rSignature = signature[0:64]
        rSignatureBinary = self.base16_to_binary(rSignature)
        rHashedData = self.hash(rSignatureBinary, 'keccak')
        secret = rHashedData[0:60]
        # key and passphrase
        sSignature = signature[64:128]
        sSignatureBinary = self.base16_to_binary(sSignature)
        sHashedData = self.hash(sSignatureBinary, 'keccak')
        key = sHashedData[0:32]
        passphrase = sHashedData[32:62]
        self.apiKey = self.uuid_format_key(key)
        self.secret = self.base16_to_urlencode_base64(secret)
        self.password = self.base16_to_urlencode_base64(passphrase)

    def base16_to_urlencode_base64(self, string):
        return self.urlencode_base64(self.binary_to_base64(self.base16_to_binary(string)))

    def check_required_credentials(self, error=True):
        if self.privateKey:
            return True
        starkKeysProvided = self.check_stark_key_credentials(False)
        apiKeysProvided = self.check_api_key_credentials(False)
        if not starkKeysProvided or not apiKeysProvided:
            if error:
                raise AuthenticationError(self.id + ' requires either a privateKey credential(Ethereum Key Authentication) or all of the following credentials: apiKey, secret, password, starkPrivateKey')
            else:
                return False
        return True

    def check_stark_key_credentials(self, error=True):
        keys = ['starkPrivateKey']
        starkKeysProvided = self.check_credentials(keys, False)
        if not starkKeysProvided and self.privateKey:
            self.derive_stark_private_key(self.privateKey)
        return self.check_credentials(keys, error)

    def check_api_key_credentials(self, error=True):
        keys = ['apiKey', 'secret', 'password']
        apiKeysProvided = self.check_credentials(keys, False)
        if not apiKeysProvided and self.privateKey:
            self.recover_default_api_credentials(self.privateKey)
        return self.check_credentials(keys, error)

    def check_credentials(self, keys, error=True):
        for i in range(0, len(keys)):
            key = keys[i]
            if self.requiredCredentials[key] and not getattr(self, key):
                if error:
                    raise AuthenticationError(self.id + ' requires "' + key + '" credential or a privateKey credential(Ethereum Key Authentication)')
                else:
                    return False
        return True
    ...
    def handle_errors(self, code, reason, url, method, headers, body, response, requestHeaders, requestBody):
        ...
        errors = self.safe_string(response, 'errors', [])
        if errors is not None:
            ...
            for i in range(0, len(errors)):
                ...
                paramHint = self.safe_string(response, 'param')
                self.throw_exactly_matched_exception(self.exceptions['exact'], message, feedback)
                self.throw_exactly_matched_exception(self.exceptions['exact'], message, paramHint)
                self.throw_broadly_matched_exception(self.exceptions['broad'], body, feedback)

    def parse_array_transpose(self, obj):
        result = {}
        keys = list(obj.keys())
        for i in range(0, len(keys)):
            key = keys[i]
